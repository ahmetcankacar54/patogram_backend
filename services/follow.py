...
from schemas.follow import FollowBase, SetFollowBase


async def set_follow(follow: SetFollowBase, current_user: int, db: Session = Depends(get_db)):
    ...
    if (follow_request.follow_status == 1):
        new_follow = Follow(user_id=current_user,
                            post_id=follow_request.post_id, isFollow=follow_request.follow_status)
        db.add(new_follow)
        db.commit()
